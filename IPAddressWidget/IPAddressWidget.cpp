...
void IPAddressWidget::paintEvent(QPaintEvent *event)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        painter.drawPoint(width, height() / 2 + 2);
    }
}

int IPAddressWidget::getIndex(QLineEdit *pEdit)
{
    int index = -1;
    for (int i = 0; i < 4; i++)
    {
...
bool IPAddressWidget::eventFilter(QObject *obj, QEvent *ev)
{
...
void IPAddressWidget::setText(const QString &strIP)
{
...
bool IPAddressWidget::isTextValid(const QString &strIP)
{
...
QString IPAddressWidget::text() const
{
    QString strIP;
    for (int i = 0; i < 4; i++)
    {
